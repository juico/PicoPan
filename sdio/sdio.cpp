...
#define SDIO_DATA_SM 2
#define SDIO_IRQ_SM 3
#define SDIO_DMA_CH 10
#define SDIO_DMA_CHB 11
...
enum sdio_pio_state { SDIO_PIO_RX, SDIO_PIO_TX };
static struct {
...
sdio_status_t rp2040_sdio_command_R1(uint8_t command, uint32_t arg,
  ...
  while (pio_sm_get_rx_fifo_level(SDIO_PIO, SDIO_CMD_SM) < wait_words) {
    if ((uint32_t)(millis() - start) > 2) {
      ...
      {
        printf("Timeout waiting for response in rp2040_sdio_command_R1(%d), "
               "PIO PC: %d RXF: %d TXF: %d\n",
               (int)command,
               (int)pio_sm_get_pc(SDIO_PIO, SDIO_CMD_SM) -
                   (int)g_sdio.pio_cmd_clk_offset,
               (int)pio_sm_get_rx_fifo_level(SDIO_PIO, SDIO_CMD_SM),
               (int)pio_sm_get_tx_fifo_level(SDIO_PIO, SDIO_CMD_SM));
      }
  ...
  if (response) {
    ...
    if (crc != actual_crc) {
      printf("rp2040_sdio_command_R1(%d): CRC error, calculated %X packet has "
             "%X\n",
             (int)command, crc, actual_crc);
...
sdio_status_t rp2040_sdio_command_R2(uint8_t command, uint32_t arg,
  ...
  while (dma_channel_is_busy(SDIO_DMA_CH)) {
    if ((uint32_t)(millis() - start) > 2) {
      printf("Timeout waiting for response in rp2040_sdio_command_R2(",
             (int)command, "), ", "PIO PC: ",
             (int)pio_sm_get_pc(SDIO_PIO, SDIO_CMD_SM) -
                 (int)g_sdio.pio_cmd_clk_offset,
  ...
  if (crc != actual_crc) {
    printf("rp2040_sdio_command_R2(", (int)command, "): CRC error, calculated ",
  ...
  if (response_cmd != 0x3F) {
    printf("rp2040_sdio_command_R2(%d): Expected reply code 0x3F",
           (int)command);
    return SDIO_ERR_RESPONSE_CODE;
  }
...
sdio_status_t rp2040_sdio_command_R3(uint8_t command, uint32_t arg,
  ...
  while (pio_sm_get_rx_fifo_level(SDIO_PIO, SDIO_CMD_SM) < 2) {
    if ((uint32_t)(millis() - start) > 2) {
      printf("Timeout waiting for response in rp2040_sdio_command_R3(%d), PIO "
             "PC: %d RXF:%d TXF: %d\n ",
             (int)command,
             (int)pio_sm_get_pc(SDIO_PIO, SDIO_CMD_SM) -
                 (int)g_sdio.pio_cmd_clk_offset,
...
sdio_status_t rp2040_sdio_rx_start_var(uint8_t *buffer, uint32_t num_blocks,
                                       uint16_t block_size) {
  // Buffer must be aligned
  assert(((uint32_t)buffer & 3) == 0 && num_blocks <= SDIO_MAX_BLOCKS);
  ...
  while (g_sdio.blocks_checksumed < g_sdio.blocks_done && maxcount-- > 0) {
    ...
    if (checksum != expected) {
      ...
      if (g_sdio.checksum_errors == 1) {
        printf("SDIO checksum error in reception: block %d, calculated %X "
               "expected %X \n",
...
sdio_status_t rp2040_sdio_rx_poll(uint32_t *bytes_complete) {
  ...
  if (g_sdio.transfer_state == SDIO_IDLE) {
    ...
    printf("rp2040_sdio_rx_poll() timeout, "
           "PIO PC:%d, RXF: %d, TXF: %d, DMA CNT: %d\n",
...
sdio_status_t check_sdio_write_response(uint32_t card_response) {
  ...
  if (wr_status == 2) {
    ...
    printf("SDIO card reports write CRC error, status %d\n", card_response);
    return SDIO_ERR_WRITE_CRC;
  } else if (wr_status == 6) {
    printf("SDIO card reports write failure, status %d\n", card_response);
    return SDIO_ERR_WRITE_FAIL;
  } else {
    printf("SDIO card reports unknown write status %d\n", card_response);
    return SDIO_ERR_WRITE_FAIL;
  }
}
 ...
 */
  ...
  if (g_sdio.transfer_state == SDIO_IDLE) {
    ...
    printf("rp2040_sdio_tx_poll() timeout, PIO PC: %d, RXF: %d, TXF: %d, DMA "
           "CNT: %d\n",
           (int)pio_sm_get_pc(SDIO_PIO, SDIO_DATA_SM) -
...
static void load_rx() {
  if (g_sdio.pio_state == SDIO_PIO_TX) {
    pio_remove_program(SDIO_PIO, &sdio_data_tx_program,
                       g_sdio.pio_data_tx_offset);
    // Data reception program
    g_sdio.pio_data_rx_offset =
        pio_add_program(SDIO_PIO, &sdio_data_rx_program);
    g_sdio.pio_cfg_data_rx =
    ...
    sm_config_set_clkdiv_int_frac(&g_sdio.pio_cfg_data_rx, g_sdio.clock_divider,
                                  0);
    g_sdio.pio_state = SDIO_PIO_RX;
  }
}
static void load_tx() {
  if (g_sdio.pio_state == SDIO_PIO_RX) {
    ...
    sm_config_set_clkdiv_int_frac(&g_sdio.pio_cfg_data_tx, g_sdio.clock_divider,
                                  0);
    g_sdio.pio_state = SDIO_PIO_TX;
  }
}
void rp2040_sdio_init(int clock_divider) {
  g_sdio.clock_divider = clock_divider;
  ...
  if (!resources_claimed) {
    ...
    pio_sm_claim(SDIO_PIO, SDIO_IRQ_SM);
    if (dma_channel_is_claimed(SDIO_DMA_CH)) {
      printf("First channel already claimed");
    }
    if (dma_channel_is_claimed(SDIO_DMA_CHB)) {
      printf("Second channel already claimed");
    }
  ...
  pio_enable_sm_mask_in_sync(SDIO_PIO,
                             (1u << SDIO_IRQ_SM) | (1u << SDIO_CMD_SM));
