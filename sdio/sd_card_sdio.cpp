...
static bool logSDError(int line)
{
    ...
    printf("SDIO SDIO SD card error on line %d error code %d\n", line,(int)g_sdio_error);
    return false;
}
...
bool SdioCard::writeSector(uint32_t sector, const uint8_t* src)
{
    ...
    // {
    //   printf("SDIO SdioCard::writeSector(%X) failed: %d\n", sector, (int)g_sdio_error);
    // }
...
bool SdioCard::writeSectors(uint32_t sector, const uint8_t* src, size_t n)
{
    if (((uint32_t)src & 3) != 0)
    {
        printf("WriteSectors not aligned\n");
    ...
    if (g_sdio_error != SDIO_OK)
    {
      printf("SDIO SdioCard::writeSectors(%X,...,%d) failed: %d\n",sector,  (int)n,(int)g_sdio_error);
        stopTransmission(true);
        return false;
    }
    else
...
bool SdioCard::readSector(uint32_t sector, uint8_t* dst)
{
    ...
    if (g_sdio_error != SDIO_OK)
    {
      printf("SDIO SdioCard::readSector(%X) failed: %d\n", sector, (int)g_sdio_error);
    }
...
bool SdioCard::readSectors(uint32_t sector, uint8_t* dst, size_t n)
{
    ...
    if (g_sdio_error != SDIO_OK)
    {
      printf("SDIO SdioCard::readSectors(%X,...,%d) failed: %d\n",sector,  (int)n,(int)g_sdio_error);
        stopTransmission(true);
        return false;
    }
    else
